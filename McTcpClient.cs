        ...
        private void StartClient(string user, string uuid, string sessionID, string server_ip, ushort port, int protocolversion, ForgeInfo forgeInfo, bool singlecommand, string command)
        {
            ...
            if (!singlecommand)
            {
                if (botsOnHold.Count == 0)
                {
                    ...
                    BotLoad(new ChatBots.SyraxSettings());
                    if (Settings.fbankenabled) { BotLoad(new ChatBots.SyraxBot()); }
                    if (Settings.serverchatenabled) { BotLoad(new ChatBots.SyraxChatHook()); }
                    if (Settings.wallsenabled) { BotLoad(new ChatBots.SyraxWalls()); }
